    ...
    vtkInformationVector* outputVector)
{
  ...
  outInfo->Set(CAN_PRODUCE_SUB_EXTENT(), 1);
	...
	int number_of_ncdf_variables;
	nc_inq_nvars(this->NCDFFD, &number_of_ncdf_variables);

	this->Internals->Variables.resize(number_of_ncdf_variables+1);  // Plus one for generated wind
	this->Internals->number_of_exposed_variables = 0;
	
	// For every variable in the file
	for(int i=0;i<number_of_ncdf_variables;i++)
		{
    int nd;
	...
	// and expose wind
	strcpy(this->Internals->Variables[number_of_ncdf_variables].name, "wind");
	this->Internals->Variables[number_of_ncdf_variables].mapped_index = -2;	// no matching netcdf variable
    ...
    vtkInformationVector* outputVector  )
{
  ...
  int ph_id  = -1;
  int phb_id = -1;
	for (int i = 0; i < this->Internals->Variables.size(); i++)
  ...
  // fprintf(stderr, "lat and lon loaded\n");

	OGRSpatialReference oUTM, *oLL;
	oUTM.SetProjCS("UTM 16 (WGS84)");
  ...
  for(size_t i = 0; i < this->Internals->Variables.size();i++)
    {
